...
# Load reference captions from the text file
reference_captions = {}
with open("data/textFiles/tokens_clean.txt", "r") as file:
    for line in file:
        data = ast.literal_eval(line.strip())
        reference_captions.update(data)
...
def predict_caption(image_paths):
    ...
    for image_path in image_paths:
        ...
        # Fetch reference captions for the image ID
    ...
    return preds, references  # Return both generated captions and reference captions
...
def upload():
    ...
    # Perform image captioning
    # Perform image captioning
    captions, references = predict_caption([destination])

# Initialize a list to store BLEU scores for each caption
    all_bleu_scores = []

# Compute BLEU scores for each generated caption against each reference caption
    for caption in captions:
        bleu_scores = []
        for reference in references:
        # Compute BLEU score using NLTK
            bleu_score = sentence_bleu([reference.split()], caption.split(), smoothing_function=SmoothingFunction().method1)
            bleu_scores.append(bleu_score)
        all_bleu_scores.append(bleu_scores)

# Combine captions with their corresponding BLEU scores
        caption_bleu_pairs = list(zip(captions, all_bleu_scores))

# Render the "upload.html" template and pass data to it
    return render_template("upload.html", img_name=filename, caption_bleu_pairs=caption_bleu_pairs)
